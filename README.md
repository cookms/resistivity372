...
Blocks can be added, edited, moved, duplicated, and deleted. Each row describes the command
an operator will see in execution order. The builder expands the list into ordinary
version-1 YAML and validates the complete result with the configured `SafetyLimits`.
Generated YAML must be saved and validated before run initialization, exactly like a
manually authored sequence. Builder metadata stored in the YAML allows the high-level block
list to be restored when the file is loaded again; the existing `steps` list remains the
authoritative executable representation for `SequenceRunner`.

The main window gives run setup/status, the sequence builder, the live graph, and the
application log separate top-level tabs so each workspace can use the full window area.
